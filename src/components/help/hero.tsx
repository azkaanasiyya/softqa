...
export default function HelpHero() {
    ...
    const handleCommonSearch = (text: SetStateAction<string>) => {
        setSearchQuery(text);
    };
            ...
            <FadeInSection className="max-w-[1192px] flex flex-col gap-10 md:gap-12 lg:gap-20 justify-center items-center w-full">
                <FadeInSection delay={0.5} className="flex flex-col gap-8 md:gap-10 w-full max-w-[784px]">
                    ...
                    <div className="flex flex-col items-center justify-center w-full gap-4 overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row gap-2 md:gap-4 items-start md:items-center md:justify-center w-full">
                            <span className="text-[14px] md:text-[16px] leading-[22px] md:leading-6 text-grayscale-900">Common searches:</span>
                            <div className="flex flex-row gap-2 md:gap-4 items-center justify-start md:justify-center overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] md:overflow-visible whitespace-nowrap w-full max-w-[487px]">
                                <div 
                                    className="bg-grayscale-50 border hover:bg-grayscale-200 cursor-pointer border-grayscale-100 rounded-[8px] py-[5px] md:py-2 px-3 text-[14px] md:text-[16px] leading-[22px] md:leading-6 text-grayscale-900 whitespace-nowrap"
                                    onClick={() => handleCommonSearch("Reset my password?")}
                                >
                                    Reset my password?
                                </div>
                                <div 
                                    className="bg-grayscale-50 border hover:bg-grayscale-200 cursor-pointer border-grayscale-100 rounded-[8px] py-[5px] md:py-2 px-3 text-[14px] md:text-[16px] leading-[22px] md:leading-6 text-grayscale-900 whitespace-nowrap"
                                    onClick={() => handleCommonSearch("Subscription plans")}
                                >
                                    Subscription plans
                                </div>
                                <div 
                                    className="bg-grayscale-50 border hover:bg-grayscale-200 cursor-pointer border-grayscale-100 rounded-[8px] py-[5px] md:py-2 px-3 text-[14px] md:text-[16px] leading-[22px] md:leading-6 text-grayscale-900 whitespace-nowrap"
                                    onClick={() => handleCommonSearch("Bug Detection")}
                                >
                                    Bug Detection
                                </div>
                            </div>
                    ...
                    <FadeInSection delay={0.5} className="bg-grayscale-50 rounded-[16px] flex flex-col pt-0 md:pt-1 pb-2 md:pb-4 px-4 md:px-6 gap-2 max-w-[376px] w-full">
                        ...
                        <div className="flex flex-col gap-1 w-full">
                            <div className="flex flex-row justify-between items-center py-3">
                                ...
                                <ArrowRight className="w-4 h-4 text-grayscale-400 hover:text-primary-500 cursor-pointer"/>
                            </div>
                            <div className="flex flex-row justify-between items-center py-3">
